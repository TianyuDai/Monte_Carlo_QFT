// Exact value of correlation function
//#include <fstream>
#include <iostream>
#include <math.h>
#include <stdlib.h>
using namespace std; 
//ofstream fout <"exact_corr">; 
const int LX = 8, LT = 8, N = LX * LT; 
...
// matrix is singular
bool inverse(int M[N][N], float inverse[N][N])
{
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			if (i == j || abs(i - j) == LT || abs(i - j) == 1) M[i][j] = 1; 
			else M[i][j] = 0; 
		}
	}
