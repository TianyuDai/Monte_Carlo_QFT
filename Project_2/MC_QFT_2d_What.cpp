...
#include <algorithm>
#include <random>
std::ofstream fout("block_2d_g1_k01_8-48"); 
std::ofstream fout2("Corr_Func_g1_k01_8-48"); 

const size_t d = 2, LT = 48, LX = 8; 
int dims[d] = {LX, LT}; 
const int nblock = 10000, blocks = 10000, nSign = 65; 
double g = 0.01, kappa = 0.249376559, sigma = 1; 
	...
	for (size_t i = 0; i < nblock; i++)
	{
		...
		for (size_t j = 0; j < blocks; j++)
		{
			...
			for (int k = 0; k < nSign; k++)
			{
				...
				//std::cout<< c<< std::endl; 
				for (int l = 0; l < c; l++)
					if (visited[l][1] == LT/2) {corr_block[i] += chi[0][0] * chi[visited[l][0]][visited[l][1]]; std::cout<<1; }
			}
		}
		//time_t timer2; 
		//time(&timer2); 
